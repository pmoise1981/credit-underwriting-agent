...
**Rate limiting.** 24 cases × N_REPEATS is enough sequential Bedrock traffic to trip on-demand throttling — this actually happened in CI during development (a `ThrottlingException` that exhausted the retry budget mid-run). Three changes came out of that, all in `config.py` unless noted: the inter-call pacing is now `EVAL_CALL_PACING_SECONDS` (default 4s, env-overridable) instead of a hardcoded 2s, applied in *both* eval scripts — `eval_agentic.py` previously had no pacing at all, which mattered because it runs immediately after the fair lending eval against the same account's rate limit; the adaptive retry budget (`BEDROCK_RETRY_CONFIG`) was raised from 10 to 25 attempts; and `.github/workflows/eval.yml` now sleeps 15s between the two AWS-backed eval steps so the second one doesn't start already near the throttling edge. None of this raises the account's actual Bedrock throughput quota, though — if the on-demand limit itself is the bottleneck, these reduce how often a run gets throttled but can't guarantee it never will; a persistent problem would need a quota increase on the AWS side.
