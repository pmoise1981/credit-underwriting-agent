...
**Rate limiting.** 24 cases × N_REPEATS is enough sequential Bedrock traffic to trip on-demand throttling — this actually happened in CI during development (a `ThrottlingException` that exhausted the retry budget mid-run). Several changes came out of that, all in `config.py` unless noted: the inter-call pacing is now `EVAL_CALL_PACING_SECONDS` (default 4s, env-overridable) instead of a hardcoded 2s, applied in *both* eval scripts — `eval_agentic.py` previously had no pacing at all, which mattered because it runs immediately after the fair lending eval against the same account's rate limit; and `.github/workflows/eval.yml` now sleeps 15s between the two AWS-backed eval steps so the second one doesn't start already near the throttling edge.

The adaptive retry budget (`BEDROCK_RETRY_CONFIG`) is env-controlled rather than just raised outright, because the *same* `llm` client this config applies to is also what `lambda_handler.py` uses for real production requests, and that Lambda has a hard 60-second timeout (`terraform/lambda.tf`). A large retry budget is safe for the eval suite, which has no deadline, but the same budget in production risks a sustained-throttle retry loop getting hard-killed by Lambda mid-backoff instead of returning a clean error. So `BEDROCK_MAX_ATTEMPTS` defaults to the original, Lambda-safe 10 attempts, and only CI opts into 25 via an env var set at the job level in `eval.yml` — production is unaffected.

None of this raises the account's actual Bedrock throughput quota, though — if the on-demand limit itself is the bottleneck, these reduce how often a run gets throttled but can't guarantee it never will; a persistent problem would need a quota increase on the AWS side.
